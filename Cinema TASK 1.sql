...
VALUES('?tirilmi? ?eher','2022-04-13')
INSERT INTO Films(Name,Releasedate)
...
INSERT INTO Films(Name,Releasedate)
VALUES('Tecili yard?m','2022-03-31')
SELECT * FROM Films
------------------------------------------------------------------------
...
SELECT * FROM Tickets
-------------------------------------------------------------------------
CREATE PROCEDURE usp_BuyTicket @HallId int
AS
SELECT * FROM Tickets Where HallId=@HallId

EXEC usp_BuyTicket @HallId =2
------------------------------------------------------------------------
CREATE FUNCTION GetEmptySeat(@HallId int)
RETURNS int 
AS
Begin
     DECLARE @Count int
	 SELECT @Count =COUNT(*) FROM Tickets WHERE HallId=@HallId
	 RETURN @Count
END

SELECT dbo.GetEmptySeat(2)